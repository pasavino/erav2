...
import { View, Text, StyleSheet, FlatList, ActivityIndicator, RefreshControl, Modal, TouchableOpacity } from 'react-native';
...
type Vehicle = {
  ...
  Modelo?: string; ModelDesc?: string;
  ColorHex?: string; CodigoColor?: string;
};
...
export default function Cart() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true);
      await ensureOk(requestForm('/ax_delete_vehiculos.php', {
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={styles.row}>
              ...
              <TouchableOpacity onPress={() => askDelete(item)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
          ...
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalBtns}>
              <TouchableOpacity
                ...
                disabled={deleting}
                onPress={() => { if (!deleting) setConfirmItem(null); }}
                hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
              >
                <Text style={styles.btnGhostText}>Cancel</Text>
              </TouchableOpacity>

              {/* No asumimos que <Boton> tenga 'disabled'; lo desactivamos con pointerEvents */}
              <View style={[deleting && styles.btnDisabled, { borderRadius: 10 }] } pointerEvents={deleting ? 'none' : 'auto'}>
...
const styles = StyleSheet.create({
  ...
  card: { borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 12, padding: 16, alignItems: 'center', gap: 12 },
  muted: { color: '#6b7280' },

  row: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f0f0f0', gap: 12 },
  colorDot: { width: 16, height: 16, borderRadius: 8, borderWidth: 1, borderColor: '#e5e7eb' },
  name: { fontSize: 16, fontWeight: '600' },
  subtitle: { color: '#6b7280' },

  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.35)', justifyContent: 'center', alignItems: 'center', padding: 24 },
  modalCard: { width: '100%', maxWidth: 420, backgroundColor: '#fff', borderRadius: 16, padding: 18, gap: 10 },
  modalIconRow: { alignItems: 'center', marginBottom: 4 },
  modalTitle: { fontSize: 18, fontWeight: '700', textAlign: 'center' },
  modalMsg: { textAlign: 'center', color: '#4b5563' },

  modalBtns: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: 12, marginTop: 12 },
  btnGhost: { paddingVertical: 10, paddingHorizontal: 14, borderRadius: 10, borderWidth: 1, borderColor: '#e5e7eb', alignItems: 'center', justifyContent: 'center', minHeight: 44 },
